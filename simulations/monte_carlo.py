...
class MonteCarlo:
    ...
    def get_direction(self, game: Game) -> int:
        """Chooses fix direction and then runs simulation with random direction
        until game is over.
        ...
        if game.game_possible_movement(game.matrix):
            for _ in range(self.iteration_cnt + 1):
                ...
                while (
                    game.move_in_direction_possible(fixed_dir, inner_matrix)
                ) == False:
                    fixed_dir = dirs[random.randint(0, 3)]
            ...
            direction = max(self.score_all, key=self.score_all.get)
